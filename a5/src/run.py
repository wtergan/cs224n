...
if args.variant == 'vanilla':
    ...
    # Config synthesizer to True, create model.
...
if args.function == 'pretrain':
    ...
    # Usage of pretrain hyperparameters for pretraining of the model.
    hyperparameters = {
        "max_epochs": 650,
        "batch_size": 128,
        "learning_rate": 6e-3,
    ...
    # Initialization of the training config, then training using the pretrian dataset.
    training_config = trainer.TrainerConfig(**hyperparameters)
    trainer.Trainer(model, pretrain_dataset, None, training_config).train()

    # Saving of the finetuned model parameters to the specified path.
    ...
    # If pre-trained model parameters given in reading_params_path, load it into the model for usage,
    # use hyperparameters specified with the pretrained model.
    ...
    # Otherwise, usage of hyperparameters specified without the pre-trained model.
    else:
        hyperparameters = {
            "max_epoch": 75,
            "batch_size": 256,
    ...
    # Initialization of the name dataset from the corpus, which will be used for finetuning (training set).
    finetune_corpus = open(args.finetune_corpus_path).read()
    finetune_dataset = dataset.NameDataset(pretrain_dataset, finetune_corpus)

    # If eval_corpus_path is provided, initialize the name dataset from the corpus for evaluation (testing set).
    ...
    # Otherwise, set the evaluation set as None.
    else:
        eval_dataset = None

    # Training config, training.
    training_config = trainer.TrainerConfig(**hyperparameters)
    trainer.Trainer(model, finetune_dataset, eval_dataset, training_config).train()
