...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        # will implement the "dense" variant of the Synthesizer attention, which involves only computing the 
        # value vector (not the query and key vectors), and using a small feed forward network that will 
        # learn a deterministic mapping from the input sequence length to the attention weights.

        # calculation of the values for all the heads in batch, and moving of head forward to be the batch dim.
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)

        # calculation of the feed forward network that represents the synthesizer.
        # (B, nh, T, hs) * (1, 1, hs, T) -> (B, nh, T, T)
        ffn_1 = F.relu(self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2))
        ffn_2 = ffn_1 @ self.w2[:, :T] + self.b2[:T]
        
        # atttention calculation.
        att = ffn_2.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10)
        att = F.softmax(att, dim=1)
        att = self.attn_drop(att)
        y = att @ v
