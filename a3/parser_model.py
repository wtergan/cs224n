...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        """
        - Initialization of the weights used in tandem with the embedded input, of the shape (num_features * embedded_size, hidden_size).
        - Initialization of the biases used in tandem with the linear transformation of the embedded input and its corresponding weights,
          of shape (hidden_size, )
        - Application of Xavier's initialization to the weights to mitigate exploding or vanishing gradients. (Variance same across each layer)
        - Application of unifrom distribution initialization of the biases.
        """
        ...
        # Construction of the dropout layer, using the dropout probabiltiy parameter provided.
        self.dropout = nn.Dropout(p=dropout_prob)

        """
        - Initialization of the weights used in tandem with the output of the preceeding layer, used to produce the logits (unnormalized scores),
          of the shape (hidden_size, n_classes).
        - Initialization of the biases used in tandem with the linear transformation of the output of the preceeding layer and its corresponding
          weights, of the shape (n_classes, )
        - Application of Xavier's initialization to the weights to mitigate exploding or vanishing gradients. (Variance same across each layer)
        - Application of unifrom distribution initialization of the biases.
        """
    ...
    def embedding_lookup(self, w):
        ...
        """
        - Select the embeddings corresponding to the word indices in w. Result is the shape of (batch_size, n_features, embed_size)
        - Reshape the result of those embeddings to be (batch_size, n_features * embed_size).
        - This result will now be the embedded input used in the model.
        """
        x = self.embeddings[[w]].view(len(w), -1)
    ...
    def forward(self, w):
        ...
        """
        - Creation of the embedded input, used for the forward pass of the model.
        - Affine (linear) transformation of the embedded input and its corresponding weights, whose result is summed with its corresponding
          bias. Of the shape (batch_size, hidden_size).
        - RELU non-linear activation function applied to the result of the linear transformation. Of the shape (batch_size, hidden_size).
        - Dropout applied after the RELU non-linear activation function.
        - Affine (linear) transformation of the result of the RELU non-linear activation function and its corresponding weight, whose result is
          summed with its corresponding bias. Results is logits, the unnormalized scores. Of the shape (batch_size, n_classes).
        """
        embedded_input = self.embedding_lookup(w)
        h = F.relu(embedded_input.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        logits = self.dropout(h).matmul(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
